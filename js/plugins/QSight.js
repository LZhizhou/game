...
var Galv = Galv || {};          // Galv's main object (added for gobbers hoard)
Galv.DETECT = Galv.DETECT || {}
...
(function() {
  ...
  // ADDED FOR GOBBERS HOARD
  //Detects distance of 2 coordinates
  Galv.DETECT.dist = function(x1,y1,x2,y2) {
    return Math.sqrt(Math.pow(x1 - x2,2) + Math.pow(y1 - y2,2));
  };

  //ADDED FOR GOBBERS HOARD
  // Actual function to call to check distance
  Galv.DETECT.event = function(id,dist,los,itemID) {
    return $gameMap.event(id).distDetect(dist,los,itemID);
  };

  // ADDED FOR GOBBERS HOARD
  Game_Character.prototype.distDetect = function(range,los,itemID) {
    var itemID = itemID || 0;
    var range = range * 48;   //var range = range * Galv.DETECT.tile;
    var target = itemID > 0 ? $gameMap.event(itemID) : $gamePlayer;
    var x1 = this.screenX();
    var y1 = this.screenY();
    ...
    if (dist <= range) { // If in radius range of target and not undetectable
      if (los) {
